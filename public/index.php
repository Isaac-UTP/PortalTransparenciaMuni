...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['archivo'])) {
    $tipos = $_POST['tipos'] ?? '';
    $anio = $_POST['anio'] ?? ''; // deberia ser anno
    $descripcion = $_POST['descripcion'] ?? '';
    $numero = $_POST['numero'] ?? '';
    $archivo = $_FILES['archivo'];

    // Verificar si el archivo es válido
    if ($tipos && $anio && $descripcion && $numero && $archivo['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($archivo['tmp_name'], $rutaDestino)) {
            ...
            $sql = "INSERT INTO documentos (tipos, anno, descripcion, numero, link) 
                    VALUES (:tipos, :anio, :descripcion, :numero, :link)";
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':tipos', $tipos);
            $stmt->bindParam(':anio', $anio);
            $stmt->bindParam(':descripcion', $descripcion);
            $stmt->bindParam(':numero', $numero);
            $stmt->bindParam(':link', $rutaDestino);
